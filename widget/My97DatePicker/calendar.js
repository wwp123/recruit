...
My97DP.prototype = {
    ...
    isDisableMinute: function(p,pv){
        var _pVal;
        switch (p){
            case 'H':
                _pVal = $dp.disabledHours;
                break;
            case 'm':
                _pVal = $dp.disabledMinutes;
                break;
            case 's':
                _pVal = $dp.disabledSeconds;
                break;
        }
        if(_pVal==null){
            return true;
            ...
            var a = true;
            if (_pVal && _pVal.length > 0) {
                for (var B = 0; B < _pVal.length; B++) {
                    if(_pVal[B]==pv){
                        a = false;
                    }
                }
    ...
    _f: function(p, max, c, r, e, isR) {
        ...
        for (var i = 0; i < r; i++) {
            ...
            for (var j = 0; j < c; j++) {
                ...
                else if (this.isDisableMinute(p,$dt[p]) && ( this.checkValid($dt, p) || ($dp.opposite && "Hms".indexOf(p) == -1 && this.checkRange($dt, p) == 0))) {
